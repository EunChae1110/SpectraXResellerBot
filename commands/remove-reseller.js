...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('remove-reseller')
    .setDescription('解除经销商绑定')
  ...
  async execute(interaction, { db, config, logMonitor }) {
    ...
    if (!isOwner && !isMarketManager) {
      return interaction.reply({ content: '权限不足，仅限Owner或市场经理。', flags: MessageFlags.Ephemeral });
    }

    const targetUser = interaction.options.getUser('user', true);
    const email = interaction.options.getString('email', true).trim().toLowerCase();

    let monitorMessage = `指令 /remove-reseller 发起者 <@${interaction.user.id}> 目标 <@${targetUser.id}> 邮箱 ${email}`;
    ...
    try {
      ...
      await interaction.reply({ content: `已解除 ${targetUser} 的经销商绑定。`, flags: MessageFlags.Ephemeral });
